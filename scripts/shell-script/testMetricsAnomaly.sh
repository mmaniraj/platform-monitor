...
function checkAnomaly(){
  ...
  count=$(mysql --silent -h ip-172-31-28-247.ec2.internal -P 3306 -u root -pbdre@1234 -D PlatformMonitor<<<"select count(*) as return_code from MetricsAnomalyRange2 where $2 between lowerlimit and upperlimit AND metricsName='$1' AND host='$3'")
...
file="/home/ec2-user/platform-monitor/scripts/shell-script/anomalyResult_"$today".txt"

if [ ! -f "/home/ec2-user/platform-monitor/scripts/shell-script/mailTrigger.txt" ]
then
  echo $(date +%s) > mailTrigger.txt
fi

if [ -f "/home/ec2-user/platform-monitor/scripts/shell-script/anomalyResult_"$today".txt" ] 
then
